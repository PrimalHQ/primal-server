...
domains = Accumulator{String, Int}()
@time for (i, eid) in enumerate(mdeids)
    yield(); running[] || break
    running[] || break
    i % 1000 == 0 && print("$i/$(length(mdeids))    \r")
    try
...
@time for (i, eid) in enumerate(mdeids)
    yield(); running[] || break
    i % 1000 == 0 && print("$i/$(length(mdeids))/$(length(users))    \r")
    try
        md = cache_storage.events[eid]
        local d = JSON.parse(md.content)
        if haskey(d, "nip05")
            name, domain = parse_nip05(d["nip05"])
            domain == "nostr-vip.top" && continue
        end
        follows = Set()
