...
function run_dvm_checks(randomized_delays=true)
    ...
    asyncmap(Main.App.get_dvm_feeds_all(est); ntasks=8) do eid
        ...
        if e.kind == 31990
            ...
            # @show (dvm_id, Dates.unix2datetime(e.created_at))
            # return
            ...
            LOG[] && println("run_dvm_checks: $(Nostr.hex(e.pubkey)) $dvm_id $(isnothing(r) ? "X" : "+") $tdur s")
            if isnothing(r)
                badcnt[] += 1
                push!(bads, dvm_id)
    ...
    (; good=goodcnt[], bad=badcnt[], goods, bads)
end
