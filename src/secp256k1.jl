...
function generate_keypair(; seckey::Union{Nothing,Vector{UInt8}}=nothing)
    with_context() do ctx
        ...
        GC.@preserve keypair pubkey serialized_pubkey begin
            while true
                if !isnothing(seckey) && (@ccall :libsecp256k1.secp256k1_keypair_create(ctx::Ptr{Cvoid}, pointer(keypair)::Ptr{UInt8}, pointer(seckey)::Ptr{UInt8})::Cint) != 0
                    break
                end
